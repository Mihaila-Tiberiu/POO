...
public:
    ...
    cardDebit(const cardDebit& c)
    {
        cout << "Apel constructor de copiere" << endl;

        for (int i = 0; i < 16; i++)
            this->cifreNrCard[i] = c.cifreNrCard[i];
        for (int i = 0; i < 3; i++)
            this->cifreCVV[i] = c.cifreCVV[i];

        this->nrPlati = c.nrPlati;
        this->platiInLei = new double[c.nrPlati];
        for(int i = 0; i < c.nrPlati; i++)
            this->platiInLei[i] = c.platiInLei[i];

        this->numeDetinator = new char[strlen(c.numeDetinator) + 1];
        strcpy(this->numeDetinator, c.numeDetinator);

        this->dataExpirare.zi = c.dataExpirare.zi;
        this->dataExpirare.luna = c.dataExpirare.luna;
        this->dataExpirare.an = c.dataExpirare.an;
    }
    ...
    {
        delete[] this->platiInLei;
        this->platiInLei = new double[nrPlati];
        this->nrPlati = nrPlati;
        for (int i = 0; i < nrPlati; i++)
            this->platiInLei[i] = platiInLei[i];
    ...
    double* getPlatiInLei()
    {
        ...
        // NU ESTE SAFE PENTRU CA SE POT FACE MODIFICARI PRIN GETTER (se returneaza o adresa)!
        // putem rezolva prin introducerea unei copii, dar apare un memory leak

        double* copie = new double[this->nrPlati];
        for (int i = 0; i < this->nrPlati; i++)
            copie[i] = this->platiInLei[i];

        return copie;
    }

    // getter platiInLei de pe o anumita pozitie
    double getPlatiInLei(int i)
    {
        return this->platiInLei[i];
    }
...
int main()
{
    ...
    cout << "A doua plata a cardului este: " << c1.getPlatiInLei(1) << endl;
